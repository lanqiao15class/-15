	 ...
	 var   _tlist= _wrapteacherList();
		...
		//===================================================
				$('#<%=inputid%>').searchableSelect({data:_tlist,
					afterSelectItem: function(sval,stext)
				 	{
						if($("em[name=cepTeacherId]").html()=="点击填写"||sval==""){
							return;
						}else{
							var data="lqClassId=${lqclassInfo.lq_class_id}&cepTeacherId="+sval;
							//提交
				   			$.ajax({
						    	  type:"post",
						             ...
						             success:function(data){
						         		if(data.code == 200){
						         			$("em[name=cepTeacherId]").html(stext);
						         			layer.msg("修改成功");
						         		}else{
						          			layer.msg("修改失败");
						          		}
		...
		%>
				$('#<%=inputid%>').searchableSelect({data:_tlist,selectid:'<%=selectid%>', selecttext:'<%=selecttext%>',
					afterSelectItem: function(sval,stext)
				 	{
						if($.trim($("em[name=cepTeacherId]").html())==stext||sval==""){
							return;
						}else{
							var data="lqClassId=${lqclassInfo.lq_class_id}&cepTeacherId="+sval;
							//提交
